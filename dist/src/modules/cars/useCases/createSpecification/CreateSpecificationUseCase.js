...
let CreateSpecificationUseCase = class CreateSpecificationUseCase {
    ...
    execute({ name, description }) {
        return __awaiter(this, void 0, void 0, function* () {
            const isSpecificationAlreadyRegistered = yield this.specificationRepository.findByName(name);
            if (isSpecificationAlreadyRegistered) {
                throw new AppError_1.AppError("Specification already registered");
            }
            yield this.specificationRepository.create({ name, description });
        });
    }
};
