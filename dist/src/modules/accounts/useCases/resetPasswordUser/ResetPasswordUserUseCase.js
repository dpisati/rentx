...
let ResetPasswordUserUseCase = class ResetPasswordUserUseCase {
    ...
    execute({ token, password }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const validDate = this.dateProvider.compareIfBefore(userToken.expires_date, this.dateProvider.dateNow());
            if (validDate) {
                throw new AppError_1.AppError("Expired token");
            }
            const user = yield this.usersRepository.findById(userToken.user_id);
            if (!user) {
                throw new AppError_1.AppError("Invalid user");
            }
